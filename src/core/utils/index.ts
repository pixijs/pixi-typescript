...
namespace core.utils {
    import DATA_URI = core.DATA_URI;
    import RETINA_PREFIX = core.settings.RETINA_PREFIX;
    import URL_FILE_EXTENSION = core.URL_FILE_EXTENSION;
    import SVG_SIZE = core.SVG_SIZE;
    import VERSION = core.VERSION;
    let nextUid = 0;
    let saidHello = false;
...
// };
     ...
     */
    export function hex2rgb(hex: number, out?: ArrayLike<number>) {
        out = out || [];
    ...
    export function hex2string(hex: number) {
        let hex_ = hex.toString(16);
        hex_ = '000000'.substr(0, 6 - hex_.length) + hex;
     ...
     */
    export function getResolutionOfUrl(url: string) {
     ...
     */
    export function decomposeDataUri(dataUri: string): any {
     ...
     */
    export function sayHello(type: string) {
        if (saidHello) {
            return;
        }
    ...
    export function isWebGLSupported() {
        ...
        try {
            ...
            let gl = (canvas.getContext('webgl', contextOptions) || canvas.getContext('experimental-webgl', contextOptions)) as WebGLRenderingContext;
